...
class ModelConverter():
    ...
    def __init__(self,
        ...
        make a graph static. To start the conversion process call self.save_static_graph.
        ...
        """
        self.src, self.dst = self.check_paths(src_dir, dst_dir)
        ...
        self.graph = self.model.signatures[serving_key]  # the concrete function (graph) of the model
    ...
    def check_paths(self, src: str, dst: str) -> tuple(Path, Path):
    ...
    def _static_tensorspecs(self, batch_size: int) -> dict[tf.TensorSpec]:
        ...
        for name, input_layer in input_layers.items():
            ...
            tensorspecs[name] = tf.TensorSpec(shape=tf.TensorShape(shape),
    ...
    def create_static_signatures(self: Sequence[int]) -> dict[tf.types.experimental.ConcreteFunction]:
        ...
        for batch_size in self.batch_sizes:
            ...
            signature_dictionary[save_serving_key] = tf.function(self.graph).get_concrete_function(**static_tensorspec)
